...
//            ]
        ...
        $response = Curl::to(sprintf("%s/jobs/%s?api-version=%s&timeout=20", $this->batch_url, $params['jobId'], $this->api_version))
            ->withHeader(sprintf("Authorization: SharedKey %s:%s", $this->batch_account, $signature))
            ->withHeader('Date: ' . $batch_date)
            ->withHeader('Content-Type: application/json;odata=minimalmetadata')
            ->withHeader('Content-Length: ' . $content_len)
...
//        Log::debug(print_r($response, true));
    ...
    /**
     * return completed status of job
     *
     * @param $job_id
     * @return bool
     */
    ...
    /**
     * return failed status of job
     *
     * @param $job_id
     * @return bool
     */
    public function isFailedJob($job_id)
    {
        $job = $this->getJob($job_id);

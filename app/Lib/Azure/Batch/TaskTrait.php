...
//        Log::debug(print_r($response, true));
     ...
     * @return bool
     */
    public function reactiveTask($params)
    {
        $batch_date = $this->getDate();
        ...
        $response = Curl::to(sprintf("%s/jobs/%s/tasks/%s/reactivate?api-version=%s&timeout=20", $this->batch_url, $params['jobId'], $params['taskId'], $this->api_version))
            ->withHeader(sprintf("Authorization: SharedKey %s:%s", $this->batch_account, $signature))
            ->withHeader('Date: ' . $batch_date)
            ->withHeader('Content-Type: application/json;odata=minimalmetadata')
            ->withHeader('Content-Length: ' . $content_len)
     ...
     * @return array
     */
    public function listTask($job_id, $filter=null, $skiptoken=null)
    {
        $batch_date = $this->getDate();
        ...
        $data = sprintf("GET\n\n\n\n\n\n%s\n\n\n\n\n\n/%s/jobs/%s/tasks%s%s\napi-version:%s\ntimeout:20",
            ...
            $this->batch_account,
            $job_id,
            (is_null($filter) === false ? "\n\$filter:" . $filter : ''),
            (is_null($skiptoken) === false ? "\n\$skiptoken:" . $skiptoken : ''),
            $this->api_version
        );

        $signature = $this->signatureString($data);

        $response = Curl::to(sprintf("%s/jobs/%s/tasks?%s%sapi-version=%s&timeout=20",
            $this->batch_url,
            $job_id,
            (is_null($filter) === false ? '$filter=' . urlencode($filter) . '&'  : ''),
            (is_null($skiptoken) === false ? '$skiptoken=' . urlencode($skiptoken) . '&' : ''),
            $this->api_version
            ))
            ->withHeader(sprintf("Authorization: SharedKey %s:%s", $this->batch_account, $signature))
            ->withHeader('Date: ' . $batch_date)
        ...
        if ($response->status == 200) {
            ...
            if (empty($tasks) &&
                empty($contents['odata.nextLink']) === false
            ) {
                parse_str($contents['odata.nextLink'], $params);

                if (empty($params['$skiptoken']) === false) {
                    return $this->listTask($job_id, $filter, $params['$skiptoken']);
                }
            }

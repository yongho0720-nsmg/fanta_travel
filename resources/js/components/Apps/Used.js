...
class Used extends Component {
    ...
    setChart = (refresh = false) => {
        ...
        //console.log(items.map(val=> ({'name':val.name, 'data': val.items.map(cnt => cnt.count)})))
        this.setState({
    ...
    render() {
        ...
        const breadcrumb = (
            <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item">앱</li>
                <li className="breadcrumb-item active"><strong>잔존 수</strong></li>
            </ol>
        );

        const contentHead = (
            <div className="card-header">
                <div className="float-left mt-2"><i className='' style={{'marginRight':'3px'}}> </i>잔존 수</div>
        ...
        const contentItems = (
            <table className="table table-responsive-sm table-striped table-hover table-outline mb-0 table-bordered app-table">
                ...
                <tr>
                    <th className="text-center middle">앱</th>
                    {label.map((val,idx) => {
                        return (
...
if (document.getElementById('pf_react_used')) {
    ReactDOM.render(<Used />, document.getElementById('pf_react_used'));
}
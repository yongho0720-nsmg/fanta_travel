...
@section('content')
    ...
    <form  class="form-inline" method="POST" onsubmit="return check_value()" action="{{url('/admin/banned_words')}}">
        {{ csrf_field() }}
        <div id="search_box" class="w-50  mt-0 mb-0">
            <div class="form-group box w-100 px-3 d-inline-block">
                <h6 class="font-weight-bold ml-5  mt-2">금칙어</h6>
    ...
    <script>
        ...
        $(document).ready(function(){
            $(".btn_banned_word").on('click', function () {
                ...
                if (confirm(word + ' 을(를) 금칙어에서 삭제 하시겠습니까?')) {
                    $.ajax({
                        url: '/admin/banned_words/{id}',
                        type: 'delete',
                        headers: {
